(function () {
  ...
  function updatePrevNextButtons() {
    ...
    elBtnNext.disabled =
      currentIndex < 0 || currentIndex >= ruqyahItems.length - 1;
  }
  ...
  function playRuqyah(item, btn) {
    ...
    var idx = ruqyahItems.findIndex(function (x) {
      return x.surahId === item.surahId;
    });
    if (idx >= 0) playByIndex(idx);
  }
  var fullRuqyahUrl =
    "https://download.tvquran.com/download/selections/68/590e615aa1d3f.mp3";
  var fullRuqyahUrl2 =
    "//download.tvquran.com/download/TvQuran.com__Rouqia/TvQuran.com__r06.mp3";
  var fullRuqyahUrl3 =
    "https://download.tvquran.com/download/TvQuran.com__Rouqia/TvQuran.com__r07.mp3";
  ...
  function playNext() {
    if (currentIndex >= 0 && currentIndex < ruqyahItems.length - 1)
  ...
  fetch(API_RECITERS)
    ...
    .then(function (data) {
      if (data.reciters && data.reciters.length) {
        var reciter =
          data.reciters.find(function (r) {
            return (
              r.moshaf &&
              r.moshaf[0] &&
              r.moshaf[0].surah_list &&
              r.moshaf[0].surah_list.split(",").indexOf("1") >= 0
            );
          }) || data.reciters[0];
        var server =
          reciter.moshaf && reciter.moshaf[0] ? reciter.moshaf[0].server : null;
        if (server) {
          render(server);
        } else {
          elList.innerHTML =
            '<p class="ruqyah-intro">تعذر تحميل القراء. تحقق من الاتصال.</p>';
        }
      } else {

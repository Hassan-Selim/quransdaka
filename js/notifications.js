/**
 * Project: Quran Sadaka (قرآن صدقة)
 * Developer: Hassan Selim
 * Full Notifications & Prayer Monitoring Script
 */
...
function firstRandomAzkar() {
    if (!sessionStorage.getItem("firstVisitThisSession")) {
        ...
        if (Notification.permission === "granted") {
            // إشعار متصفح عادي كـ Fallback
            new Notification("ذكر اليوم 🌙", options);
        }
...
function hourlyAzkar() {
    const msg = azkarList[Math.floor(Math.random() * azkarList.length)];
    sendNotification("ذكر الساعة 🌙", msg, "/azkar");
}
setInterval(hourlyAzkar, 1000 * 60 * 60);

// ======= 4. جدولة أذكار الصباح والمساء =======
...
// ======= 5. مراقبة مواقيت الصلاة (الـ Logic المطور) =======
...
setInterval(() => {
    ...
    Object.entries(currentPrayerTimes).forEach(([prayerName, timeObj]) => {
        ...
        const prayerTimeString = prayerDate.toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit' });
        ...
        if (currentTimeString === prayerTimeString && lastNotifiedPrayer !== prayerName) {
            ...
            // إرسال أمر للـ Service Worker (كما طلبت في كودك)
    ...
    // إعادة ضبط التنبيه عند نهاية الساعة (من كودك الأصلي)
    if (now.getMinutes() === 59) {
        lastNotifiedPrayer = null;
    }
}, 30000); // فحص كل 30 ثانية للأداء

// ======= 6. التشغيل عند التفاعل =======

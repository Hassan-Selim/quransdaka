!(function () {
  ...
  ((window.shareVerse = function (e, t, n, a) {
    ...
    const sNum = sessionStorage.getItem("target_surah");
    const aNum = sessionStorage.getItem("target_ayah");
    
    if (!sNum || !aNum) return;

    // التأكد من استدعاء دالة فتح السورة أولاً
    if (typeof V === "function") {
        V(parseInt(sNum));
    }

    let attempts = 0;
    const maxAttempts = 50; // زيادة عدد المحاولات قليلاً

    const scrollInterval = setInterval(() => {
        // البحث عن الآية بـ ID محدد (يفضل أن يكون الكود المولد للسورة يضع هذا الـ ID)
        const targetId = `ayah-${aNum}`;
        let element = document.getElementById(targetId) || 
                      document.querySelector(`[data-ayah="${aNum}"]`);
        ...
        if (attempts > maxAttempts) {
            clearInterval(scrollInterval);
            console.log("الآية لم تظهر في الـ DOM بعد 50 محاولة");
        }
    }, 200);
}),
    (window.doSearch = function (e) {
      const t = ee(e);
      t && window.saveToHistory(t);
    }),
    ...
    if (searchInput && searchInput.value.length > 1) {
        window.saveToHistory(searchInput.value);
        searchInput.blur();
    }
    ...
    if (swarBtn) {
        ...
        // ننتظر قليلاً حتى يبدأ القسم في التحميل ثم نبدأ مراقبة الآية
        setTimeout(() => {
            window.checkAndOpenTarget();
        }, 300);
    }
    }),
